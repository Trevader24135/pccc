...
struct BranchMetric<F: num_traits::Float> {
...
impl<F: num_traits::Float> BranchMetric<F> {
...
impl<F: num_traits::Float> Default for BranchMetric<F> {
    fn default() -> Self {
...
#[derive(Debug)]
pub(crate) struct StateMachine<F: num_traits::Float> {
    /// Code polynomials
...
impl<F: num_traits::Float> StateMachine<F> {
...
#[derive(Debug)]
struct BetaCalculator<F: num_traits::Float> {
...
impl<F: num_traits::Float> BetaCalculator<F> {
...
#[derive(Debug)]
struct AlphaCalculator<F: num_traits::Float> {
    /// Number of encoder states
    num_states: usize,
    /// Decoding algorithm to use
    decoding_algo: DecodingAlgo,
...
impl<F: num_traits::Float> AlphaCalculator<F> {
...
#[derive(Debug)]
pub(crate) struct DecoderWorkspace<F: num_traits::Float> {
...
impl<F: num_traits::Float> DecoderWorkspace<F> {
...
fn bit_metric_from_llr<F: num_traits::Float>(bit: Bit, llr_val: F) -> F {
    let two = F::one() + F::one();
    match bit {
...
pub(crate) fn encode<F: num_traits::Float>(
    info_bits: &[Bit],
...
pub(crate) fn decode<F: num_traits::Float>(
    code_bits_llr: &[F],
...
fn run_bcjr_backward_pass<F: num_traits::Float>(
    code_bits_llr: &[F],
    info_bits_llr_prior: &[F],
    state_machine: &mut StateMachine<F>,
    workspace: &mut DecoderWorkspace<F>,
...
fn run_bcjr_forward_pass<F: num_traits::Float>(
    code_bits_llr: &[F],
    info_bits_llr_prior: &[F],
    state_machine: &mut StateMachine<F>,
    workspace: &mut DecoderWorkspace<F>,
...
fn compute_previous_beta_values<F: num_traits::Float>(
    code_bits_llr: &[F],
    input_bit_llr_prior: F,
    state_machine: &mut StateMachine<F>,
    workspace: &mut DecoderWorkspace<F>,
...
fn compute_next_alpha_values<F: num_traits::Float>(
    code_bits_llr: &[F],
    input_bit_llr_prior: F,
    state_machine: &mut StateMachine<F>,
    workspace: &mut DecoderWorkspace<F>,
...
fn maxstar<F: num_traits::Float>(x: F, y: F, decoding_algo: DecodingAlgo) -> F {
...
fn linear_log_map_correction_term<F: num_traits::Float>(abs_diff: F) -> F {
...
fn log_map_correction_term<F: num_traits::Float>(abs_diff: F) -> F {
    (-abs_diff).exp().ln_1p()
}
